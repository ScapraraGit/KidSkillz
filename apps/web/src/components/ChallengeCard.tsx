      ...
      {today.length > 0 && (
        ...
        >
          ...
          <ul className="space-y-3">
            {today.map((r) => (
              <ChallengeRow key={r.challenge.id} row={r} variant={variant} />
            ))}
          </ul>
        </Card>
      )}
  ...
  return (
    <Tooltip
      label={
          ...
          ? "Done! Bonus credits already in your balance."
          : `Earn ${ch.rewardCredits} credits when you reach ${ch.target}.`
      }
    >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {done ? (
              ...
              <span className="text-xs text-slate-500 shrink-0">
                {variant === "YOUNGER"
                  ? `${value}/${ch.target}`
                  : `${value}/${ch.target} · +${ch.rewardCredits}🪙`}
              </span>
            )}
          </div>
          <div className="mt-1">
