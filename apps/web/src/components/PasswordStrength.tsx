 ...
 */
export function PasswordStrength({ value, identifiers = [] }: { value: string; identifiers?: string[] }) {
  ...
  return (
    <div className="text-xs space-y-1">
      <div className="flex gap-1 h-1.5">
        {[0, 1, 2, 3, 4].map((i) => (
          <div key={i} className={`flex-1 rounded ${i < filled ? COLORS[score] : "bg-slate-200"}`} />
        ))}
      </div>
      <div className="flex justify-between text-slate-500">
        ...
        <span>
          {score >= 3
            ? "Looks good."
            : r.feedback.warning || r.feedback.suggestions[0] || "Try a longer pass-phrase"}
        </span>
      </div>
    </div>
  );
